...
def recursion(battlefield: list[list[int]]) -> int:
    n, m = len(battlefield), len(battlefield[0])
    i, j = 0, 0
    is_all_cell_capture = True
    while i < n:
        while j < m:
            if battlefield[i][j] == 1:
                battlefield[i][j] = 2
            if is_all_cell_capture and battlefield[i][j] == 0:
                is_all_cell_capture = False
            j += 1
        j = 0
        i += 1

    if is_all_cell_capture:
        return 1
