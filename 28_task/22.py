def SherlockValidString(s: str) -> bool:
    ...
    d_count_value: dict[int, list[str]] = {}
    ...
    if (
        len(d_count_value[key_min]) == 1
        and d_char_count[d_count_value[key_min][0]] - 1
        == d_char_count[d_count_value[key_max][0]]
    ):
        return True
    if (
        len(d_count_value[key_min]) == 1
        and d_char_count[d_count_value[key_min][0]] - 1 == 0
    ):
        return True
    if (
        len(d_count_value[key_max]) == 1
        and d_char_count[d_count_value[key_max][0]] - 1 == 0
    ):
        return True
