...
class SimpleGraph:
    ...
    def _is_index_in_adjacency(self, i_to_check_1: int, i_to_check_2: int) -> bool:
        if i_to_check_1 >= len(self.vertex) or i_to_check_2 >= len(self.vertex):
            return False
        return self.vertex[i_to_check_1] is not None and self.vertex[i_to_check_2] is not None
    ...
    def RemoveVertex(self, v: int) -> None:
        ...
        for relation_index, _ in enumerate(self.m_adjacency[v]):
            ...
            self.RemoveEdge(v1=relation_index, v2=v)
        self.vertex[v] = None
    ...
    def is_connected_graph(self):
        ...
        Краткое название: "Проверка, является ли неориентированный граф связным."
        Сложность: size - O(n) / time - O(n)

        Рефлексия:
            Идея моего решения очень простая:
                "Если из одного узла графа можно дойти до любого другого узла - значит граф связный".
            За произвольный узел взял первый узел графа и проверяю его связь с остальными.
        """
        for vertex_i in range(self.max_vertex):
            if not self.DepthFirstSearch(v_from=0, v_to=vertex_i):
                return False
        return True
...
Краткое название: "Поиск самого длинного простого пути в ориентированном графе без циклов"
Сложность: size - O(n) / time - O(n)
