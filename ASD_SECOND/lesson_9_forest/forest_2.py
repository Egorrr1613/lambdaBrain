...
class SimpleTree:
    ...
    def AddChild(self, parent_node: SimpleTreeNode | None, new_child: SimpleTreeNode) -> None:
    ...
    def balance_tree(self) -> None:
        ...
        Краткое название: "Балансировка четного двоичного дерева"
        Сложность: size - O(n) / time - O(n)

        Рефлексия:
            Кажется, зная что у нас четное двоичное дерево можно было бы получить глубину каждого листа
                и далее переставлять "выбивающиеся" листья от максимально длинной ветке к максимально короткой.
            Однако такая логика показалась слишком сложной.
            Решил пойти "простым" путем, пересоздавая все дерево, учитывая признак "бинарности",
                добавляя к каждому узлу только 2 потомка.
        ...
        self.Root = all_nodes.pop(0)
        parents_node_q = [self.Root]
        while all_nodes:
            parents_node_q.append(all_nodes.pop(0))
            self.AddChild(parent_node=parents_node_q[0], new_child=parents_node_q[-1])
            if all_nodes:
                parents_node_q.append(all_nodes.pop(0))
                self.AddChild(parent_node=parents_node_q[0], new_child=parents_node_q[-1])
            parents_node_q.pop(0)

    def get_sub_tree_count(self, find_subtree_node: SimpleTreeNode) -> int:
        """
        Задание: №9
        Номер задачи из задания: №3
        Краткое название: "Получить количество четных поддеревьев для узла"
        Сложность:

        Рефлексия:

        """
        all_sub_nodes = self._recursion_get_all_node(
            all_find_nodes=[find_subtree_node], node_children_check=find_subtree_node
        )
        break_nodes_list = []
        for node in all_sub_nodes:
            break_nodes_list += self.get_break_list(node)

        return len(break_nodes_list) // 2
